...
class App extends Component {
   ...
   async  componentDidMount() {
        ...
        if (settings != null) {
            this.setState({
                settings: {...settings, timer: parseFloat(settings.timer)},
    ...
    onSettingsSave() {
        ...
        this.setState((state) => ({
            ...
            settings: { ...state.settingsInputs, timer: parseFloat(state.settingsInputs.timer) }
        }));
        window.saveSettings(this.state.settings);
    }
...
class TimeLogger extends Component {
    ...
    async onSubmit() {
        ...
        const resp = await window.logTime(Object.keys(keys), parseFloat(this.state.timeToLog))
    ...
    async onAddKey() {
        ...
        let extraIssues = [];
        if (extraKeys.length) {
            extraIssues = await window.loadIssues(`issuekey in (${extraKeys.join(',')})`);
        }
    ...
    async loadJiraIssues() {
        try {
            const issues = await window.loadIssues(this.props.jql);
            let extraIssues = [];
            if (this.state.extraKeys.length) {
                extraIssues = await window.loadIssues(`issuekey in (${this.state.extraKeys.join(',')})`);
            }
    ...
    render() {
        ...
        return (
            <div className="Settings">
                ...
                <div>
                    Apikey: <input type="text" onChange={(e) => this.props.onChange('apikey', e.target.value)} value={apikey}></input>
                </div>
                <div>
